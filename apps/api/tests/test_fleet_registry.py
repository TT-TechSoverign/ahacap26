...
def test_fleet_hierarchy_and_cardinality():
    assert len(SUBMASTER_REGISTRY) == 8, f"Expected 8 submasters, got {len(SUBMASTER_REGISTRY)}"
    assert len(AGENT_REGISTRY) == 33, f"Expected 33 agents in AGENT_REGISTRY, got {len(AGENT_REGISTRY)}"
    assert len(AGENT_RUNNERS) == 33, f"Expected 33 agent runners, got {len(AGENT_RUNNERS)}"
...
def test_all_submasters_have_valid_sops_and_unique_children():
    ...
    # Exactly 33 agents assigned with zero duplicates
    assert len(all_supervised_agents) == 33
    assert len(set(all_supervised_agents)) == 33
